...
def makeTestPattern(patch_width=9, nbits=256):
    ...
    '''
    s = patch_width  # use the symbol in BRIEF
    iid_gaussian = np.floor(np.random.randn(nbits * 4) * s * s / 25 + s / 2).astype(np.int32)
    iid_gaussian[iid_gaussian > s - 1] = s - 1
    iid_gaussian[iid_gaussian < 0] = 0
    compareX = iid_gaussian[:nbits] * s + iid_gaussian[nbits:2 * nbits]
    compareY = iid_gaussian[2 * nbits:3 * nbits] * s + iid_gaussian[3 * nbits:]
    #compareX = np.random.randint(s*s, size=nbits)
    #compareY = np.random.randint(s*s, size=nbits)
...
def computeBrief(im, gaussian_pyramid, locsDoG, k, levels,
    ...
    for x, y, c in locsDoG:
        if x+offset_pl<0 or x+offset_pr>w or y+offset_pl<0 or y+offset_pr>h:
            continue
        patch = gaussian_pyramid[y + offset_pl:y + offset_pr, x + offset_pl:x + offset_pr, c].flatten()
        feature = patch[compareX] < patch[compareY]
        locs.append((x,y,c))
        desc.append(feature.astype(np.int32))
...
def briefLite(im):
    ...
    compareX, compareY = np.load('testPattern.npy')
    locs, desc = computeBrief(im, gaussian_pyramid, locsDoG, None, None, compareX, compareY)
    return locs, desc
